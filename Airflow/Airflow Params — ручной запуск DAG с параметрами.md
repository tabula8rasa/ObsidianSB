...
# 7. Валидация формата даты через `pattern`
...
Param(
    ...
    type="string",
    pattern=r"^$|^\d{4}-\d{2}-\d{2}$",
    description="Дата в формате YYYY-MM-DD. Можно оставить пустой.",
)
```
...
| `|` | или |
| `^\d{4}-\d{2}-\d{2}$` | строка вида `2026-01-10` |
...
# 8. Валидация логики дат внутри задачи

`pattern` проверяет только формат строки.

Но он не проверит бизнес-логику:

```text
end_date >= start_date
```

Поэтому такую проверку лучше делать в Python:
...
def resolve_period(**context) -> dict[str, str]:
    ...
    start_date_raw = params.get("start_date") or ds
    end_date_raw = params.get("end_date") or start_date_raw
...
# 10. Пример с флагом `force_reload`
...
# 11. Пример с ограничением количества дней
...
# 12. Доступ к `params` в разных местах
...
# 14. Важный нюанс: scheduled run и manual run

DAG должен корректно работать в двух режимах.
...
DAG использует введённый период.
...
# 15. Почему важно не плодить дубли

Ретро-загрузка часто означает повторную загрузку данных за уже существующий период.

Плохой сценарий:

```text
В хранилище уже есть данные за 2026-01-10
DAG снова вставил данные за 2026-01-10
Получились дубли
```

Для production-пайплайнов лучше делать загрузку идемпотентной.

**Идемпотентность** — это свойство операции, при котором повторный запуск с теми же параметрами даёт тот же итоговый результат, а не портит данные.

---

# 16. Как сделать загрузку идемпотентной

## Вариант 1. Delete + Insert

Перед вставкой удалить данные за период:
...
- просто;
- понятно;
- хорошо для партиционированных витрин.

Минусы:

- на больших таблицах может быть дорого;
- нужно аккуратно работать с транзакциями.

## Вариант 2. Insert overwrite partition

Если таблица партиционирована по дате, можно перезаписывать партиции:

```text
перезаписать только партиции за нужный период
```
...
## Вариант 3. Upsert / Merge

Использовать `MERGE` или `ON CONFLICT`.

Идея:

```text
если запись уже есть → обновить
если записи нет → вставить
```

Подходит для таблиц с ключами.

## Вариант 4. Версионирование загрузок
...
И переключать актуальную версию данных.

Полезно, если важна история пересчётов.

---

# 17. Пример полного пайплайна с перезаписью периода
...
# 19. Когда это особенно полезно

## Пересчёт витрины после изменения логики

Было:

```text
метрика считалась старым способом
```

Стало:

```text
метрика считается новым способом
```

Код выкатили, но старые данные остались посчитаны по-старому.
...
## Источник обновил исторические данные

Например, Postgres-таблица получила корректировки за прошлый месяц.

Решение:

```text
перезагрузить только изменённый период
```

## Сбой в пайплайне

Например, три дня API отдавал неполные данные.

Решение:

```text
перезапустить DAG за эти даты
```

## Миграция или изменение схемы

Например, добавилось новое поле в витрине.

Решение:

```text
пересчитать исторический период после релиза
```
...
## Антипаттерн 1. Ручной запуск через Jupyter вместо Airflow

Плохо:

```text
аналитик просит инженера
инженер идёт в Jupyter
меняет даты руками
запускает Spark job
никто не видит историю запуска
```
...
## Антипаттерн 2. Нет идемпотентности

Плохо:

```text
каждый повторный запуск добавляет новые строки
```

Лучше:

```text
повторный запуск за тот же период даёт тот же результат
```

## Антипаттерн 3. Нет ограничений на период

Плохо:

```text
кто-то случайно запускает пересчёт за 5 лет
```
...
## Антипаттерн 4. Параметры без описания
...
```python
params={
    "start_date": Param(..., description="Начальная дата в формате YYYY-MM-DD"),
    "end_date": Param(..., description="Конечная дата в формате YYYY-MM-DD"),
}
```
...
`params` в Airflow позволяют превратить DAG из “чёрного ящика по расписанию” в управляемый инструмент.

Вместо того чтобы менять код, лезть в Jupyter или просить инженера руками запускать job, пользователь может открыть Airflow UI, ввести параметры и запустить DAG.

Но для production это должно быть безопасно:

```text
params + validation + idempotency + разумные лимиты
```

Тогда ручные ретро-загрузки становятся удобными, контролируемыми и воспроизводимыми.
