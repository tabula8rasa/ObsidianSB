...
# 10. Настройка NTP на Arch Linux через chrony
...
## Не держать два NTP-клиента одновременно

Перед включением `chrony` лучше отключить `systemd-timesyncd`:
...
| Параметр | Значение |
|---|---|
| `pool ge.pool.ntp.org iburst` | использовать пул NTP-серверов Грузии, быстро начать синхронизацию |
| `makestep 1.0 3` | первые 3 коррекции могут сделать резкий step, если ошибка больше 1 секунды |
| `rtcsync` | периодически синхронизировать аппаратные часы с системными |
...
Показывает общее состояние синхронизации.

```bash
chronyc sources -v
```

Показывает источники времени.
...
# 11. Как читать `chronyc sources -v`
...
^+ ntp2.example.net              2   6   377    40    +80us[  +70us] +/-  15ms
^- bad.example.net               3   6   377    38    +20ms[  +20ms] +/- 120ms
```
...
| `^` | обычный сервер |
| `=` | peer |
| `#` | локальный reference clock |

## Второй символ

| Символ | Значение |
|---|---|
| `*` | выбранный основной источник |
| `+` | хороший кандидат |
| `-` | источник отброшен алгоритмом выбора |
| `?` | нет связи или источник непонятен |
| `x` | источник считается ложным |
| `~` | слишком нестабильный источник |
...
означает, что источник выбран как основной.

---

# 12. Примеры публичных NTP-серверов
...
Лучше использовать региональные зоны:
...
> [!warning]
> Некоторые провайдеры используют **leap smear** — плавное размазывание високосной секунды.  
> Не всегда стоит смешивать leap-smear-серверы и обычные NTP-серверы в одной конфигурации.

---

# 13. NTP Pool

**NTP Pool** — это большая сеть добровольных NTP-серверов.
...
DNS возвращает несколько серверов из пула.

Плюсы:

- просто использовать;
- много серверов;
- есть географические зоны;
- хорошо подходит для обычных клиентов.

Минусы:

- это добровольная инфраструктура;
- не стоит бездумно использовать для критичных систем;
- для большого парка устройств лучше поднять свои локальные NTP-серверы.

Правильная идея для организации:

```text
много клиентов → локальный NTP-сервер → несколько внешних источников
```

А не так:

```text
10000 клиентов → напрямую в pool.ntp.org
```

---

# 14. Локальный NTP-сервер в сети
...
## Проблема: UDP/123 заблокирован

Проверить сетевые ограничения:
...
## Проблема: часы сбиваются после перезагрузки
...
Обычно для Linux лучше, чтобы RTC был в UTC:
...
# 19. Минимальная рекомендация для Arch Linux

## Для обычного ПК или ноутбука

Использовать `systemd-timesyncd`.
...
## Для сервера, виртуальной машины или нестабильной сети
...
Остальные клиенты синхронизируются с ним.

---

# 20. Ключевая мысль

NTP — это не просто “поставить правильное время”.

Это часть инфраструктуры, от которой зависят:
...
Для домашнего Arch Linux обычно достаточно `systemd-timesyncd`.  
Для сервера и серьёзной инфраструктуры лучше использовать `chrony`.
