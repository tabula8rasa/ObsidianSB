...
![[Pasted image 20260712203541.png]]
...
Метод SAGAS появился не для распределенных систем. Его создали как один из способов борьбы с **LLT (Long-lived transactions)**. 

Допустим у нас есть одна бизнес транзакция, которая должна атомарно провзаимодействовать с множеством таблиц и схем, некоторые из них заблокировать частично или полностью, однако физически транзакцию можно разбить на несколько отдельных запросов. В этом случае можно разбить одну бизнес транзакцию на несколько мелких и создать для каждой из них транзакцию противовес, которая нивелирует (а не сделает rollback) основной транзакции.
![[Pasted image 20260712204623.png]]
Тогда в позитивном сценарии все мелкие транзакции вызываются поочередно или параллельно (по правилам DAG). А в случае отказа одной из них нужно лишь вызвать компенсирующие транзакции для каждой из тех, что уже отработали до упавшей транзакции.
...
# 7. Choreography Saga
![[Pasted image 20260712210345.png]]
![[Pasted image 20260712210832.png]]
## Идея
...
![[Pasted image 20260712210719.png]]
![[Pasted image 20260712210806.png]]
## Идея
